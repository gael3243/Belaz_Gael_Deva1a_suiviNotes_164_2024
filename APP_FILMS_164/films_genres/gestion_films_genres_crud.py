...
def films_genres_afficher(id_film_genres_sel):
    print(" films_genres_afficher id_film_genres_sel ", id_film_genres_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT m.id_matiere, m.nom_matiere, tm.type_matiere
                                                    FROM t_matiere m
                                                    JOIN t_type_matiere tm ON m.fk_type_matiere = tm.id_type_matiere;"""
                if id_film_genres_sel == 0:
                    # le paramètre 0 permet d'afficher tous les films
                    # Sinon le paramètre représente la valeur de l'id du film
                    mc_afficher.execute(strsql_genres_films_afficher_data)
                else:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    valeur_id_film_selected_dictionnaire = {"value_id_film_selected": id_film_genres_sel}
                    # En MySql l'instruction HAVING fonctionne comme un WHERE... mais doit être associée à un GROUP BY
                    # L'opérateur += permet de concaténer une nouvelle valeur à la valeur de gauche préalablement définie.
                    strsql_genres_films_afficher_data += """ HAVING id_matiere= %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_genres_sel == 0:
                    flash("""La table "t_matiere" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_film_genres_sel > 0:
                    # Si l'utilisateur change l'id_film dans l'URL et qu'il ne correspond à aucun film
                    flash(f"Le film {id_film_genres_sel} demandé n'existe pas !!", "warning")
                else:
