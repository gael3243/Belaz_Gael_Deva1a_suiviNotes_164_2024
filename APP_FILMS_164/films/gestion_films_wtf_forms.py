...
class FormWTFAddFilm(FlaskForm):
    """
        Dans le formulaire "notes_ajouter_wtf.html" on impose que le champ soit rempli.
        Définition d'un "bouton" submit avec un libellé personnalisé.
    """
    nom_matiere_regexp = ""
    nom_matiere_add_wtf = StringField("Nom de la matiere ", validators=[Length(min=2, max=2000, message="min 2 max 20"),
                                                               Regexp(nom_matiere_regexp,
    ...
    fk_type_matiere_regexp = ""
    fk_type_matiere_add_wtf = StringField("Type de la matière ", validators=[Length(min=1, max=1, message="1 caractère maximum"),
                                                                        Regexp(fk_type_matiere_regexp,
                                                                               message="Pas de chiffres, de caractères "
                                                                                       "spéciaux, "
                                                                                       "d'espace à double, de double "
                                                                                       "apostrophe, de double trait union")
...
class FormWTFDeleteFilm(FlaskForm):
    ...
    """
    nom_matiere_delete_wtf = StringField("Effacer cette matiere")
    fk_type_matiere_delete_wtf = StringField("Matière")
