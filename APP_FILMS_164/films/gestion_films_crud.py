...
def films_afficher(id_film_sel):
    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_films_afficher_data = """SELECT m.id_matiere, m.nom_matiere, fk_type_matiere, tm.type_matiere
                                                    FROM t_matiere m
                                                    JOIN t_type_matiere tm ON m.fk_type_matiere = tm.id_type_matiere;"""
                if id_film_sel == 0:
                    # le paramètre 0 permet d'afficher tous les films
                    # Sinon le paramètre représente la valeur de l'id du film
                    mc_afficher.execute(strsql_films_afficher_data)
                else:
                    ...
                    strsql_films_afficher_data += """ HAVING id_matiere= %(value_id_film_selected)s"""

                    mc_afficher.execute(strsql_films_afficher_data, valeur_id_film_selected_dictionnaire)

                # Récupère les données de la requête.
                data_films_afficher = mc_afficher.fetchall()
                print("data_genres ", data_films_afficher, " Type : ", type(data_films_afficher))

                # Différencier les messages.
                if not data_films_afficher and id_film_sel == 0:
                    flash("""La table "t_matiere" est vide. !""", "warning")
                elif not data_films_afficher and id_film_sel > 0:
    ...
    print("films_afficher  ", data_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("films/films_afficher.html", data=data_films_afficher)
    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                nom_matiere_add = form_add_film.nom_matiere_add_wtf.data
                fk_type_matiere_add = form_add_film.fk_type_matiere_add_wtf.data

                valeurs_insertion_dictionnaire = {"value_nom_matiere": nom_matiere_add,
                                                  "value_fk_type_matiere": fk_type_matiere_add
                                                  }
                print("valeurs_insertion_dictionnaire ", valeurs_insertion_dictionnaire)

                strsql_insert_film = """INSERT INTO t_matiere (id_matiere,nom_matiere, fk_type_matiere)
...
"""Editer(update) un film qui a été sélectionné dans le formulaire "films_afficher.html"
Auteur : OM 2022.04.11
...
def film_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update_film.submit.data:
            ...
            return redirect(url_for('films_afficher', id_film_sel=id_film_update))
...
def film_delete_wtf():
    ...
    id_matiere_delete = request.values['id_matiere_btn_delete_html']
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_matiere WHERE id_matiere = %(value_id_matiere)s"""
            str_sql_delete_film = """DELETE FROM t_film WHERE id_matiere = %(value_id_matiere)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_matiere WHERE id_matiere = %(value_id_matiere)s"""
